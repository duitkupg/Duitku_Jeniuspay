...
## Install Duitku Jenius pay plugins through Composer
...
1. Install the plugins: `composer require duitku_payment_gateway/duitku_jeniuspay`
2. Enable the plugin:  `bin/magento module:enable Duitku_Jeniuspay`
...
6. Check the module status:  `bin/magento module:status Duitku_Jeniuspay`
...
## Install Duitku Jenius Pay plugins from Download file

With these steps, you can custom/modify our Magento plugins to handle the business model that you want

1. Download and extract the plugin you have previously downloaded from GitHub and rename the folder as Jenius Pay.
...
4. Move the Jeniuspay folders into the Magento `root/app/code/Duitku` folder. </br>(You might need to create the folder `code/Duitku` if it is not exist).
5. Run this command on terminal

    `bin/magento module:enable Duitku_Jeniuspay`
    
    `bin/magento setup:upgrade`
    
    `bin/magento cache:clean`
    
    `bin/magento module:status Duitku_Jeniuspay`


# Plugin Usage Instruction
## Basic Plugins Configuration

Before you begin, make sure that you have successfully installed and enabled Duitku Jeniuspay plugins.
Configure the Duitku Jeniuspay plugin in your Magento admin panel: 
...
4. In the Other Payment Method section, click Duitku Jeniuspay to extend Basic Settings and fill out the following fields:
